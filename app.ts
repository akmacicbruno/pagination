...
const result = document.getElementById('app') as HTMLElement;

//Ispis podataka s API-a 
let display = getPosts().then(posts => {
    ...
    var current_index1: number = 0
    var max_posts: number = parseInt(URLpageSearch.get('page'))*6
    ...
    for (var i: number = 0; i<max_posts; i++)
    {
...
function Search() {
    ...
    inputFilter.addEventListener("keyup", (e) => {
        if (e.code === "Enter") {
            ...
            var inputValue = (<HTMLInputElement>inputFilter).value, i:number;
    ...
    //URL search
    var params = new URLSearchParams(window.location.search), i:number
    ...
    if (!params.has('search') || (<HTMLInputElement>inputFilter).value.length <= 0) {
        deleteSearch();
    }
}
...
(function(){
    ...
    (<HTMLInputElement>document.getElementById("search")).value = params.get("search");
})();
